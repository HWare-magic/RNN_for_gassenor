...
parser.add_argument('-cycle_data_length', type=int, default=1000, help='choose cycle point')  ##
...
parser.add_argument('-start_con',type=int,default=1,help='start concentration div 10')
...
################ data prepare ###########
path_name =  r'C:\Users\86136\PycharmProjects\pythonProject\zy\data\waiting_process'
os.chdir(path_name)
...
##将文件按格式保存
for i in range(len(count)):
    df[i].to_csv(f'./process_result/dataset{i+start_con}0ppm_{gas}.csv')
...
for i in range(start_con, len(count) + start_con):
    ...
    for j in range(1, 5):  ## 传感器数量
        ...
        exec('ppm{}0sensor{}.to_csv(\'./process_result/{}0ppms{}_dset.csv\')'.format(i, j, i, j))
#####   数据拼接过程 并且存入到对应的文件夹中  #######
##### sensor 1 2 3 4
os.chdir(r'C:\Users\86136\PycharmProjects\pythonProject\zy\data\waiting_process\process_result')
Folder_Path =r'C:\Users\86136\PycharmProjects\pythonProject\zy\data\waiting_process\process_result'
for i in range(1,5):
...
Folder_Path1 ='C:\\Users\\86136\\PycharmProjects\\pythonProject\\zy\\data\waiting_process\\process_result\\'
for j in range(1,5):
    exec('snum{} =[]'.format(j))
